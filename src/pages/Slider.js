...
function Slider() {
    ...
    const FetchEvents = () => {
        firestore.collection("events").limit(1).get().then((querySnapshot) => {
    ...
    const FetchSeries = () => {
        firestore.collection("series").limit(1).get().then((querySnapshot) => {
            querySnapshot.forEach(element => {
                var data = element.data();
    ...
    const FetchBlogs = () => {
        firestore.collection("blogs").limit(1).get().then((querySnapshot) => {
            querySnapshot.forEach(element => {
                var data = element.data();
